...
class Node:
    ...
    def __init__(self, n, m, x, y, key, option, p=None, left=None, right=None,
        ...
        if option == '1':
            self.distance = abs(x-m+1) + abs(y-n+1)
        elif option == '2':
            self.distance = max(abs(x-m+1),abs(y-n+1))
        elif option == '3':
            self.distance = math.sqrt((x-m+1)**2 + (y-n+1)**2)
        else:
            self.distance = 0
        self.val = 2**32 - 1
